...
echo "${git_changelog}" > $propertiesFileName/changelog.txt
...
# 将空格转换为<br>标签
echo "CHANGELOG=$(cat ${propertiesFileName}/changelog.txt|awk '{printf "%s<br>",$1}')" > $propertiesFileName/gitlog.txt

# 删除临时文件changelog.txt
rm -rf $propertiesFileName/changelog.txt
